				...
				$.each(originalSettings, function (name, value) {
					...
					if ($.inArray(input.prop('type'), ['checkbox', 'radio']) > -1) {
						input.each(function () {
							...
							// Check current state
							if (ip.val() == value) {
								ip.attr('checked', 'checked').parent().addClass('checked');
							} else {
								ip.removeAttr('checked').parent().removeClass('checked');
							}
						});
					} else {
						input.val(value);
...
},{"../bower_components/dragula.js/dragula":3}],2:[function(require,module,exports){
